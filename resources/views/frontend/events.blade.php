...
@section('content')
    <div class="relative h-60 mb-10">
        ...
        <div class="relative flex items-center justify-center h-full">
            <h1 class="font-bold text-[50px] text-white tracking-wider uppercase">Event</h1>
        </div>
    </div>
    @forelse ($_news as $events)
        <div class="max-w-6xl mx-auto bg-white shadow-md rounded-lg p-6 flex flex-col gap-4 mt-2">
            <div class="flex items-start sm:items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h2 class="text-lg sm:text-xl font-semibold text-gray-800">{{ $events->title }}</h2>
            ...
            <p class="text-gray-600 text-sm">
                 {!! Str::limit(strip_tags($events->excerpt), 200) !!} <a href="{{ route('eventDetail', $events->slug) }}"
            ...
            <div class="flex justify-between items-center text-sm text-gray-500 flex-wrap gap-2">
                <span>Posted on <strong class="text-gray-800">{{ $events->created_at->format('Y d m') }}</strong></span>
                <span>Expires on <strong class="text-gray-800">{{ $events->date }}</strong></span>
            </div>
        </div>
    @empty
    @endforelse
